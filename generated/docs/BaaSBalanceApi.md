# mizancore_baas_generated.BaaSBalanceApi

All URIs are relative to *https://api.mizancore.ng*

Method | HTTP request | Description
------------- | ------------- | -------------
[**baas_balance_balance**](BaaSBalanceApi.md#baas_balance_balance) | **GET** /api/v1/baas/balance | Get settlement account balance
[**baas_balance_fee_preview**](BaaSBalanceApi.md#baas_balance_fee_preview) | **POST** /api/v1/baas/withdrawals/fee-preview | Preview withdrawal fee
[**developer_withdrawals_fee_preview**](BaaSBalanceApi.md#developer_withdrawals_fee_preview) | **POST** /api/v1/developer/withdrawals/fee-preview | Preview withdrawal fee
...
with mizancore_baas_generated.ApiClient(configuration) as api_client:
    ...
    api_instance = mizancore_baas_generated.BaaSBalanceApi(api_client)
    x_tenant_id = 'world.test.localhost' # str | Tenant identifier (UUID or domain, e.g. world.test.localhost). Required on every tenant-scoped route. Maps to the tenant whose database serves this request. In production, prefer Host-header-based resolution; X-Tenant-ID is intended for non-production environments and is rejected (HTTP 400) on production hosts.
    fee_preview_request = {"amount":100000} # FeePreviewRequest | 

    try:
        # Preview withdrawal fee
        api_response = api_instance.baas_balance_fee_preview(x_tenant_id, fee_preview_request)
        print("The response of BaaSBalanceApi->baas_balance_fee_preview:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling BaaSBalanceApi->baas_balance_fee_preview: %s\n" % e)
```
...
------------- | ------------- | ------------- | -------------
 **x_tenant_id** | **str**| Tenant identifier (UUID or domain, e.g. world.test.localhost). Required on every tenant-scoped route. Maps to the tenant whose database serves this request. In production, prefer Host-header-based resolution; X-Tenant-ID is intended for non-production environments and is rejected (HTTP 400) on production hosts. | 
 **fee_preview_request** | [**FeePreviewRequest**](FeePreviewRequest.md)|  | 
...
**200** | Successful operation. |  -  |
**422** | Validation error — amount is required and must be a positive integer in kobo |  -  |
**401** | Invalid or missing API key |  -  |
**403** | Partner account is not active |  -  |
**400** | Bad Request — tenant routing information missing or invalid (e.g. neither Host header nor X-Tenant-ID resolved to a tenant), or X-Tenant-ID supplied in production where it is disallowed. |  -  |
...
with mizancore_baas_generated.ApiClient(configuration) as api_client:
    ...
    api_instance = mizancore_baas_generated.BaaSBalanceApi(api_client)
    x_tenant_id = 'world.test.localhost' # str | Tenant identifier (UUID or domain, e.g. world.test.localhost). Required on every tenant-scoped route. Maps to the tenant whose database serves this request. In production, prefer Host-header-based resolution; X-Tenant-ID is intended for non-production environments and is rejected (HTTP 400) on production hosts.
    fee_preview_request = {"amount":150050} # FeePreviewRequest | 

    try:
        # Preview withdrawal fee
        api_response = api_instance.developer_withdrawals_fee_preview(x_tenant_id, fee_preview_request)
        print("The response of BaaSBalanceApi->developer_withdrawals_fee_preview:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling BaaSBalanceApi->developer_withdrawals_fee_preview: %s\n" % e)
```
