# mizancore_baas_generated.DeveloperPortalWebhookDeliveryLogsApi

All URIs are relative to *https://api.mizancore.ng*

Method | HTTP request | Description
------------- | ------------- | -------------
[**webhook_delivery_log_index**](DeveloperPortalWebhookDeliveryLogsApi.md#webhook_delivery_log_index) | **GET** /api/v1/developer/webhooks/deliveries | List webhook delivery logs
[**webhook_delivery_log_retry**](DeveloperPortalWebhookDeliveryLogsApi.md#webhook_delivery_log_retry) | **POST** /api/v1/developer/webhooks/deliveries/{id}/retry | Retry a failed webhook delivery
...
with mizancore_baas_generated.ApiClient(configuration) as api_client:
    ...
    api_instance = mizancore_baas_generated.DeveloperPortalWebhookDeliveryLogsApi(api_client)
    ...
    try:
        # List webhook delivery logs
        api_response = api_instance.webhook_delivery_log_index(x_tenant_id)
        print("The response of DeveloperPortalWebhookDeliveryLogsApi->webhook_delivery_log_index:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling DeveloperPortalWebhookDeliveryLogsApi->webhook_delivery_log_index: %s\n" % e)
```
...
with mizancore_baas_generated.ApiClient(configuration) as api_client:
    ...
    try:
        # Retry a failed webhook delivery
        api_response = api_instance.webhook_delivery_log_retry(id, x_tenant_id)
        print("The response of DeveloperPortalWebhookDeliveryLogsApi->webhook_delivery_log_retry:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling DeveloperPortalWebhookDeliveryLogsApi->webhook_delivery_log_retry: %s\n" % e)
```
...
**400** | Bad Request — tenant routing information missing or invalid (e.g. neither Host header nor X-Tenant-ID resolved to a tenant), or X-Tenant-ID supplied in production where it is disallowed. |  -  |
**409** | Webhook is not in a retriable state |  -  |
